...
async function fetchBitcoinData(timeframe) {
  ...
  try {
    ...
    console.error(
      `Error fetching ${timeframe} data for Bitcoin:`,
      error.message
    );
    throw error;
  }
...
async function fetchAllCoinsData(timeframe) {
  console.log(
    `Fetching ${timeframe} data for Bitcoin only using native CoinGecko intervals...`
  );

  try {
    const data = await fetchBitcoinData(timeframe);
    console.log(
      `Successfully fetched ${timeframe} data for Bitcoin (${data.length} data points)`
    );
    ...
    console.error(
      `Failed to fetch ${timeframe} data for Bitcoin:`,
      error.message
    );
    return {
      bitcoin: [],
